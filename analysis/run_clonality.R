...
run_clonality <- function(dat, Gene_study)
{

        Gene.samples <- unique(dat[dat$Gene==Gene_study,"ID"])

        Gene.clonality <- sapply(1:length(Gene.samples), function(k)
                                 {
                                         if (sum(dat$ID %in% Gene.samples[k])==1)
                                         {
                                                 clonality <- "clonal"
                                         } else 
                                         {
                                                 dat.df.Gene <- dat[dat$ID %in% Gene.samples[k],]

                                                 Gene.UCI <- max(dat.df.Gene[dat.df.Gene$Gene==Gene_study,"UCI"])
                                                 Others.LCI <- na.omit(dat.df.Gene[dat.df.Gene$Gene!=Gene_study,"LCI"])
