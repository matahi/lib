post_process_HDP <- function (out.HDP, out.merge, out.clinical=NULL, out.dir="results/", grouping=NULL, display_legend=F) {
        ...
        HDP.clusters <- c("No drivers",
                             "inv3_t3_3",
                             "NPM1",
                             "Chromatin_spliceosome",
                             "TP53_Complex",
                             "mixed fusions",
                             "inv16_t16_16",
                             "t8_21",
        ...
        for (k in 1:length(manual.clusters))
        {
                if (manual.clusters[k]=="No drivers")
                {
                        ...
                        # pdf(paste0(out.dir,"/no_drivers_manual.pdf"))
                ...
                } else if (manual.clusters[k]=="CEBPA_bi")
                {
                        ...
                        if (any(out.merge[out.HDP$class==k,"CEBPA_bi"]==0))
                        {
                                ...
                                pdf(paste0(out.dir,"/CEBPA_bi_manual.pdf"))
                                print(ggplot(tmp.df, aes(x=class,y=prob)) + geom_boxplot() + geom_jitter())
                                dev.off()
                ...
                } else if (manual.clusters[k]=="Chromatin-spliceosome")
                {
                        ...
                        pdf(paste0(out.dir,"/Chromatin_spliceosome_manual.pdf"))
                        print(ggplot(tmp.df, aes(x=class,y=prob)) + geom_boxplot() + geom_jitter())
                        dev.off()
                ...
                } else if (manual.clusters[k]=="NPM1")
                {
                        ...
                        if (any(out.merge[out.HDP$class==k,"NPM1"]==0))
                        {
                                ...
                                pdf(paste0(out.dir,"/NPM1_manual.pdf"))
                                print(ggplot(tmp.df, aes(x=class,y=prob)) + geom_boxplot() + geom_jitter())
                                dev.off()
                        }
                ...
                } else if (manual.clusters[k]=="t_8_21")
                {
                        ...
                        if (any(out.merge[out.HDP$class==k,"t_8_21"]==0))
                        {
                                ...
                                pdf(paste0(out.dir,"/t8_21_manual.pdf"))
                                print(ggplot(tmp.df, aes(x=class,y=prob)) + geom_boxplot() + geom_jitter())
                                dev.off()
                ...
                } else if (manual.clusters[k]=="inv_16")
                {
                        ...
                        if (any(out.merge[out.HDP$class==k,"inv_16"]==0))
                        {
                                ...
                                pdf(paste0(out.dir,"/inv16_manual.pdf"))
                                print(ggplot(tmp.df, aes(x=class, y=prob)) + geom_boxplot() + geom_jitter())
                                dev.off()
                ...
                } else if (manual.clusters[k]=="t_15_17")
                {
                        ...
                        if (any(out.merge[out.HDP$class==k,"t_15_17"]==0))
                        {
                                ...
                                pdf(paste0(out.dir,"/t15_17_manual.pdf"))
                                print(ggplot(tmp.df, aes(x=class, y=prob)) + geom_boxplot() + geom_jitter())
                                dev.off()
        ...
        second.prob <- Reduce("rbind",lapply(1:nrow(out.HDP$prob), function(n)
                                          {
                                                  if (any(is.na(out.HDP$prob[n,])))
                                                  {
                                                          return(data.frame(second.class=NA,second.prob=NA))
                                                  } else
                                                  {
                                                          max1 <- max(out.HDP$prob[n,])
                                                          max2 <- max(out.HDP$prob[n,-which(out.HDP$prob[n,]==max1)])
                                                          ...
                                                          return(data.frame(second.class=HDP.clusters[class2],
                                                                            second.prob=max2))
                                                  }
                                          }))
        ...
        pdf(paste0(out.dir,"/prob_diff.pdf"))
        ...
        if (is.null(grouping))
        {
                ...
                library(gplots)
                #pdf(paste0(out.dir,'/cluster_post_processed.pdf'), width=16, height=9)
                #pdf(paste0(out.dir,'/cluster_post_processed.pdf'), width=16, height=30)
                pdf(paste0(out.dir,'/cluster_post_processed.pdf'))
        ...
        } else
        {
                source("src/lib/plot/heatmap.4.R")
                library(gplots)
                pdf(paste0(out.dir,'/cluster.pdf'), width=16, height=9)
        ...
        return(list(clusters.processed=clusters.processed, second.prob=second.prob))
